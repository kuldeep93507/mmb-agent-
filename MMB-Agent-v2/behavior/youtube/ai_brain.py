...
def _get_client():
    ...
    if _client is not None:
        return _client
    # Force dotenv load using absolute path from this file's location
    try:
        from pathlib import Path
        from dotenv import load_dotenv
        # ai_brain.py → youtube/ → behavior/ → project_root/
    ...
    try:
        ...
        if not api_key:
            # Don't permanently cache False — allow retry next call
...
def scan_page_for_element(
    ...
    """
    Given a page DOM summary (text) or screenshot, ask Claude where the
    requested element is. Returns a dict with:
        found: bool
        css_selector: str | None   — best CSS selector to try
        text_to_click: str | None  — visible text of the button
        explanation: str           — what Claude found
    """
    prompt = (
    ...
    if result:
        try:
            ...
            if json_match:
                ...
                logger.info(f"[AIBrain] Page scan for '{element_description}': found={data.get('found')}, selector={data.get('css_selector')!r}")
                return data
        except Exception:
            pass
...
def identify_target_video(
    ...
    """
    Given a list of search result dicts (each with title, channel, href, video_id),
    return the INDEX of the best matching video, or None if not found.

    Each result dict should have: title, channel, href, video_id (optional)
    """
    if not results:
        return None
...
def verify_engagement(
    ...
    action: one of 'like', 'subscribe', 'autoplay_off', 'quality_360p',
                   'bell', 'comment_posted', 'dislike'

    Returns: {
        "confirmed": bool,
        "state": str,       — human-readable state description
        "suggestion": str   — what to do if not confirmed
    }
    """
    action_descriptions = {
    ...
    if result:
        try:
            ...
            if json_match:
                data = json.loads(json_match.group())
                logger.info(f"[AIBrain] Verify '{action}': confirmed={data.get('confirmed')} state={data.get('state')!r}")
                return data
        except Exception:
            pass
...
# ─────────────────────────────────────────────────────────────────────────────
# 6. AI ERROR RECOVERY — self-heals when YouTube UI changes
# ─────────────────────────────────────────────────────────────────────────────
...
def recover_from_error(
    ...
    Returns: {
        "action": str,          — 'click', 'navigate', 'search', 'wait', 'skip'
        "target": str,          — CSS selector or URL or keyword
    ...
    if result:
        try:
            ...
            if json_match:
                data = json.loads(json_match.group())
                logger.info(f"[AIBrain] Recovery: action={data.get('action')!r} target={data.get('target')!r}")
                return data
        except Exception:
            pass
...
def get_natural_watch_pattern(
    ...
    """Natural watch behavior from Claude."""
    defaults = {
