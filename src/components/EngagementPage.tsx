...
type Source = 'notification' | 'search' | 'direct';
...
export default function EngagementPage({ profiles, channels = [], getVideos, setActiveTab }: Props) {
  ...
  const [activeLaunchLimit, setActiveLaunchLimit] = useState(3);
  const [startGapMinSec, setStartGapMinSec] = useState(10);
  const [startGapMaxSec, setStartGapMaxSec] = useState(25);
  const [srcNotif,      setSrcNotif]      = useState(30);
  const [srcSearch,     setSrcSearch]     = useState(40);
  ...
  const srcDirect = Math.max(0, 100 - srcNotif - srcSearch);
                ...
                >
                  ...
                  >
                    ...
                    <span className="text-[10px] text-gray-500 bg-gray-800/80 px-1.5 py-0.5 rounded flex-shrink-0">
                      {job.source === 'notification' ? '🔔' : job.source === 'search' ? '🔍' : '🔗'} {job.source}
                    </span>
