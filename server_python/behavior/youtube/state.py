...
async def is_subscribed(tab: Any) -> bool:
    """Returns True if already subscribed. Checks VISIBLE bell (not invisible attr) or Unsubscribe button."""
    val = await _eval(tab, """
    (() => {
        // Bell button VISIBLE (parent must NOT have 'invisible' attr) = subscribed
        var bellSelectors = [
        ...
        for (var sel of bellSelectors) {
            ...
            if (parent && parent.hasAttribute('invisible')) continue;  // hidden = NOT subscribed
            var r = el.getBoundingClientRect();
            if (r.width > 4 && r.height > 4) return true;
        }
        // Check subscribe button text — "Subscribed" or aria-label "Unsubscribe"
        var sub = document.querySelector('ytd-subscribe-button-renderer button, #subscribe-button button');
        if (sub) {
            var txt = (sub.innerText || sub.textContent || '').toLowerCase().trim();
            if (txt === 'subscribed') return true;
            var al = (sub.getAttribute('aria-label') || '').toLowerCase();
...
async def get_current_time(tab: Any) -> float:
    ...
    """)
    try:
        return float(val) if val is not None else -1.0  # -1 = video element not found (distinct from real 0:00)
    except Exception:
        return -1.0
