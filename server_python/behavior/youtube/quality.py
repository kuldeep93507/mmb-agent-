...
async def _click_visible(tab: Any, selectors: list[str], rng: random.Random) -> str | None:
    """Find visible element coords and CDP-click it."""
    ...
    try:
        ...
        ok = await cdp_hover_then_click(tab, float(info["x"]), float(info["y"]), rng)
        return info.get("sel") if ok else None
    except Exception:
        return None
