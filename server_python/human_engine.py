...
async def wait_for_any_element(
    ...
    *,
    ...
    """Wait for any of multiple selectors — returns (Element, matched_selector)."""
    deadline = time.monotonic() + timeout
    while time.monotonic() < deadline:
    ...
    *,
    ...
    while i < len(chars):
        ...
        if char == " " and rng.random() < 0.40:
            # Space via insert_text so React input event fires
        ...
        # Type the correct char — insert_text fires native input event (React compatible)
        await tab.send(cdp.input_.insert_text(text=char))
        ...
        try:
            ready = await tab.evaluate(
                ...
                (() => {
                    ...
                    if (v.currentTime > 0.5) return true;
                    return false;
                })()
                """,
                return_by_value=True,
