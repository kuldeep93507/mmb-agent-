...
class PlaybackGuardian:
    ...
    async def _is_video_paused(self) -> Optional[bool]:
        try:
            result = await self._tab.evaluate(
                ...
                "if (!v) return null; "
                "if (v.ended) return null; "   # video ended naturally — don't force play
                "return v.paused; })()",
    ...
    async def _force_autoplay_off(self) -> None:
        ...
        Checks aria-checked AND aria-pressed (YT uses both in different versions).
        """
        try:
            result = await self._tab.evaluate(
                """
                (() => {
                    // Method 1: YouTube player API (internal)
                    try {
                    ...
                    // Check BOTH aria-checked and aria-pressed (different YT versions use different attrs)
                    try {
    ...
    async def verify_autoplay_off(self) -> bool:
        ...
        Checks aria-checked AND aria-pressed.
        """
        try:
            result = await self._tab.evaluate(
                """
