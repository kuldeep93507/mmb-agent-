...
# Residential US endpoint (user account — NOT proxy.smartproxy.net)
SMARTPROXY_HOST = "us.smartproxy.net"

# US states — SmartProxy uses full names in username (e.g. ALASKA, TEXAS)
US_STATES: list[str] = [
...
class SmartProxyManager:
    ...
    def __init__(self) -> None:
        ...
        # profile_id -> {username, assigned_at, state, city, life, session_id}
    ...
    def get_proxy_config(self, profile_id: str) -> dict:
        ...
        log.info(
            f"[Proxy] New session for {profile_id[:8]}...: "
            f"{session['username'][:60]}..."
        )
    ...
    def migrate_session(self, old_id: str, new_id: str) -> None:
        """After profile create: keep same proxy session on real Multilogin profile id."""
        if old_id == new_id:
            return
        sess = self._profile_sessions.pop(old_id, None)
        if sess:
            self._profile_sessions[new_id] = sess
            log.info(f"[Proxy] Session migrated {old_id[:8]} → {new_id[:8]}")

    def rotate_session(self, profile_id: str) -> dict:
        if profile_id in self._profile_sessions:
            del self._profile_sessions[profile_id]
        log.info(f"[Proxy] Force rotating session for {profile_id[:8]}...")
    ...
    async def check_proxy_health(self, profile_id: str) -> dict:
        ...
        try:
            ...
            await loop.run_in_executor(
                None,
                lambda: socket.create_connection((creds["server"], creds["port"]), timeout=5),
            )
            return {
                "ok": True,
...
def reset_proxy_manager() -> None:
    """Clear in-memory proxy sessions (after settings change or legacy format fix)."""
    global _proxy_manager
    _proxy_manager = None
